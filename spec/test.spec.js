...
describe("S3Adapter tests", () => {
  ...
  it("should throw when not initialized properly", () => {
    expect(() => {
      ...
      new Error("S3Adapter requires option 'bucket' or env. variable S3_BUCKET")
    );

    expect(() => {
      new S3Adapter("accessKey", "secretKey", {});
    }).toThrow(
      new Error("Failed to configure S3Adapter. Arguments don't make sense")
    );
  ...
  describe("configured with immutable values", () => {
    describe("not initialized properly", () => {
      ...
      it("should fail when passed an object without a bucket", () => {
        expect(() => {
          ...
          new Error(
            "S3Adapter requires option 'bucket' or env. variable S3_BUCKET"
          )
        );
      });
    });
    ...
    describe("should not throw when initialized properly", () => {
      ...
      it("should accept a second argument of object with a params object with a bucket", () => {
        expect(() => {
          new S3Adapter(
            config.get("emptyObject"),
            config.get("paramsObjectWBucket")
          );
        }).not.toThrow();
      });
  ...
  describe("getFileStream", () => {
    it("should handle range bytes", () => {
      ...
      const returnedData = {
        Body: {
          transformToWebStream: () => Buffer.from("hello world", "utf8"),
        },
      };
  ...
  describe("getFileLocation", () => {
    ...
    it("should get using the baseUrl", () => {
      ...
      expect(s3.getFileLocation(testConfig, "test.png")).toEqual(
  ...
  describe("generateKey", () => {
      ...
      options = {
        ...
        generateKey: (filename) => {
          ...
          if (lastSlash > 0) {
            ...
            key +=
              filename.substring(0, lastSlash + 1) +
              prefix +
    ...
    it("should return a file with a date stamp inserted in the path", () => {
      ...
      const response = s3
        .createFile(fileName, "hello world", "text/utf8")
        .then((value) => {
    ...
    it("should do nothing when null", () => {
      ...
      const response = s3
        ...
        .then((value) => {
          const url = new URL(value.Location);
          expect(url.pathname.substring(1)).toEqual(
            options.bucketPrefix + fileName
          );
        });
      promises.push(response);
    });
  ...
  describe("createFile", () => {
    ...
    it("should save a file with right command", async () => {
      ...
      expect(s3ClientMock.send).toHaveBeenCalledWith(
        jasmine.any(CreateBucketCommand)
      );
      expect(s3ClientMock.send).toHaveBeenCalledWith(
        jasmine.any(PutObjectCommand)
      );
    });
